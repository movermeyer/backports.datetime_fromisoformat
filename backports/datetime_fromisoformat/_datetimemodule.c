...
#define DATE_SET_MICROSECOND(o, v)          \
    (((o)->data[7] = ((v)&0xff0000) >> 16), \
     ((o)->data[8] = ((v)&0x00ff00) >> 8), ((o)->data[9] = ((v)&0x0000ff)))
...
#define TIME_SET_MICROSECOND(o, v)          \
    (((o)->data[3] = ((v)&0xff0000) >> 16), \
     ((o)->data[4] = ((v)&0x00ff00) >> 8), ((o)->data[5] = ((v)&0x0000ff)))
...
#define GET_TIME_TZINFO(p) \
    (HASTZINFO(p) ? ((PyDateTime_Time *)(p))->tzinfo : Py_None)
...
check_delta_day_range(int days)
{
    if (-MAX_DELTA_DAYS <= days && days <= MAX_DELTA_DAYS)
        return 0;
    PyErr_Format(PyExc_OverflowError, "days=%d; must have magnitude <= %d",
                 days, MAX_DELTA_DAYS);
    return -1;
}
 ...
 */
static int
normalize_datetime(int *year, int *month, int *day, int *hour, int *minute,
...
time_alloc(PyTypeObject *type, Py_ssize_t aware)
{
    ...
    self = (PyObject *)PyObject_MALLOC(aware ? sizeof(PyDateTime_Time)
...
datetime_alloc(PyTypeObject *type, Py_ssize_t aware)
{
    ...
    self =
        (PyObject *)PyObject_MALLOC(aware ? sizeof(PyDateTime_DateTime)
...
static PyObject *
new_datetime_ex(int year, int month, int day, int hour, int minute, int second,
...
static PyObject *
new_datetime_subclass_fold_ex(int year, int month, int day, int hour,
                              int minute, int second, int usecond,
                              PyObject *tzinfo, int fold, PyObject *cls)
{
    ...
    if ((PyTypeObject *)cls == &PyDateTime_DateTimeType) {
        ...
        // Subclass
        dt = PyObject_CallFunction(cls, "iiiiiiiO", year, month, day, hour,
                                   minute, second, usecond, tzinfo);
    }
                         ...
                         PyObject *cls)
{
    return new_datetime_subclass_fold_ex(year, month, day, hour, minute,
                                         second, usecond, tzinfo, 0, cls);
}

/* Create a time instance with no range checking. */
static PyObject *
new_time_ex2(int hour, int minute, int second, int usecond, PyObject *tzinfo,
             int fold, PyTypeObject *type)
{
...
static PyObject *
new_time_ex(int hour, int minute, int second, int usecond, PyObject *tzinfo,
            PyTypeObject *type)
{
...
call_tzname(PyObject *tzinfo, PyObject *tzinfoarg)
{
    ...
    result =
        _PyObject_CallMethodIdObjArgs(tzinfo, &PyId_tzname, tzinfoarg, NULL);
...
{
    static const char *const DayNames[] = {"Mon", "Tue", "Wed", "Thu",
                                           "Fri", "Sat", "Sun"};
    static const char *const MonthNames[] = {"Jan", "Feb", "Mar", "Apr",
                                             "May", "Jun", "Jul", "Aug",
                                             "Sep", "Oct", "Nov", "Dec"};
    ...
    return PyUnicode_FromFormat("%s %s %2d %02d:%02d:%02d %04d",
                                DayNames[wday],
                                MonthNames[GET_MONTH(date) - 1], GET_DAY(date),
                                hours, minutes, seconds, GET_YEAR(date));
}
 ...
 */
static int
format_utcoffset(char *buf, size_t buflen, const char *sep, PyObject *tzinfo,
                 PyObject *tzinfoarg)
{
    PyObject *offset;
    ...
    if (microseconds) {
        PyOS_snprintf(buf, buflen, "%c%02d%s%02d%s%02d.%06d", sign, hours, sep,
                      minutes, sep, seconds, microseconds);
        return 0;
    }
    if (seconds) {
        PyOS_snprintf(buf, buflen, "%c%02d%s%02d%s%02d", sign, hours, sep,
                      minutes, sep, seconds);
        return 0;
    }
...
make_Zreplacement(PyObject *object, PyObject *tzinfoarg)
{
    ...
    Zreplacement =
        _PyObject_CallMethodId(temp, &PyId_replace, "ss", "%", "%%");
    Py_DECREF(temp);
    if (Zreplacement == NULL)
        return NULL;
    ...
    while ((ch = *pin++) != '\0') {
        ...
        /* A % has been seen and ch is the character after it. */
            ...
            if (zreplacement == NULL) {
                ...
                if (tzinfo != Py_None && tzinfo != NULL) {
                    assert(tzinfoarg != NULL);
                    if (format_utcoffset(buf, sizeof(buf), "", tzinfo,
                                         tzinfoarg) < 0)
                        goto Done;
                    Py_DECREF(zreplacement);
...
build_struct_time(int y, int m, int d, int hh, int mm, int ss, int dstflag)
{
    ...
    args = Py_BuildValue("iiiiiiiii", y, m, d, hh, mm, ss, weekday(y, m, d),
                         days_before_month(y, m) + d, dstflag);
    if (args == NULL) {
        Py_DECREF(time);
        return NULL;
    }
    ...
    result =
        _PyObject_CallMethodIdObjArgs(time, &PyId_struct_time, args, NULL);
    Py_DECREF(time);
    Py_DECREF(args);
    return result;
}
...
{
    PyErr_Format(PyExc_TypeError, "can't compare %s to %s",
...
static PyObject *
multiply_truedivide_timedelta_float(PyDateTime_Delta *delta,
...
delta_add(PyObject *left, PyObject *right)
{
    ...
    if (PyDelta_Check(left) && PyDelta_Check(right)) {
        ...
        int microseconds =
            GET_TD_MICROSECONDS(left) + GET_TD_MICROSECONDS(right);
...
delta_negative(PyDateTime_Delta *self)
{
    return new_delta(-GET_TD_DAYS(self), -GET_TD_SECONDS(self),
                     -GET_TD_MICROSECONDS(self), 1);
}
...
delta_multiply(PyObject *left, PyObject *right)
{
    ...
    if (PyDelta_Check(left)) {
        ...
        result = multiply_truedivide_timedelta_float((PyDateTime_Delta *)right,
                                                     left, 0);
...
delta_divide(PyObject *left, PyObject *right)
{
    ...
    if (PyDelta_Check(left)) {
        ...
        if (PyLong_Check(right))
            ...
            result = divide_timedelta_timedelta((PyDateTime_Delta *)left,
...
delta_truedivide(PyObject *left, PyObject *right)
{
    ...
    if (PyDelta_Check(left)) {
        if (PyDelta_Check(right))
            result = truedivide_timedelta_timedelta((PyDateTime_Delta *)left,
                                                    (PyDateTime_Delta *)right);
        else if (PyFloat_Check(right))
            result = multiply_truedivide_timedelta_float(
      ...
      double *leftover)
{
    ...
    PyErr_Format(PyExc_TypeError,
                 "unsupported type for timedelta %s component: %s", tag,
                 Py_TYPE(num)->tp_name);
    return NULL;
}
...
delta_new(PyTypeObject *type, PyObject *args, PyObject *kw)
{
    ...
    static char *keywords[] = {"days",         "seconds", "microseconds",
                               "milliseconds", "minutes", "hours",
                               "weeks",        NULL};

    if (PyArg_ParseTupleAndKeywords(args, kw, "|OOOOOOO:__new__", keywords,
                                    &day, &second, &us, &ms, &minute, &hour,
                                    &week) == 0)
        goto Done;
...
delta_bool(PyDateTime_Delta *self)
{
    return (GET_TD_DAYS(self) != 0 || GET_TD_SECONDS(self) != 0 ||
            GET_TD_MICROSECONDS(self) != 0);
}
...
delta_repr(PyDateTime_Delta *self)
{
    ...
    PyObject *repr =
        PyUnicode_FromFormat("%s(%S)", Py_TYPE(self)->tp_name, args);
    Py_DECREF(args);
    return repr;
}
...
delta_str(PyDateTime_Delta *self)
{
    ...
    if (days) {
        if (us)
            return PyUnicode_FromFormat("%d day%s, %d:%02d:%02d.%06d", days,
                                        (days == 1 || days == -1) ? "" : "s",
                                        hours, minutes, seconds, us);
        else
            return PyUnicode_FromFormat("%d day%s, %d:%02d:%02d", days,
                                        (days == 1 || days == -1) ? "" : "s",
                                        hours, minutes, seconds);
    }
    else {
        if (us)
            return PyUnicode_FromFormat("%d:%02d:%02d.%06d", hours, minutes,
                                        seconds, us);
        else
            return PyUnicode_FromFormat("%d:%02d:%02d", hours, minutes,
                                        seconds);
    }
}
...
delta_getstate(PyDateTime_Delta *self)
{
    return Py_BuildValue("iii", GET_TD_DAYS(self), GET_TD_SECONDS(self),
                         GET_TD_MICROSECONDS(self));
}
...
static PyMemberDef delta_members[] = {
    ...
    {"days", T_INT, OFFSET(days), READONLY, PyDoc_STR("Number of days.")},
...
static PyGetSetDef date_getset[] = {{"year", (getter)date_year},
...
date_new(PyTypeObject *type, PyObject *args, PyObject *kw)
{
    ...
    if (PyArg_ParseTupleAndKeywords(args, kw, "iii", date_kws, &year, &month,
                                    &day)) {
...
date_local_from_object(PyObject *cls, PyObject *obj)
{
    ...
    return new_date_subclass_ex(tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday,
                                cls);
}
...
date_today(PyObject *cls, PyObject *dummy)
{
     ...
     */
    result =
        _PyObject_CallMethodIdObjArgs(cls, &PyId_fromtimestamp, time, NULL);
    Py_DECREF(time);
    return result;
}
...
date_subtract(PyObject *left, PyObject *right)
{
    ...
    if (PyDate_Check(left)) {
        if (PyDate_Check(right)) {
            /* date - date */
            int left_ord =
                ymd_to_ord(GET_YEAR(left), GET_MONTH(left), GET_DAY(left));
            int right_ord =
...
{
    return PyUnicode_FromFormat("%s(%d, %d, %d)", Py_TYPE(self)->tp_name,
...
{
    return PyUnicode_FromFormat("%04d-%02d-%02d", GET_YEAR(self),
...
{
    return build_struct_time(GET_YEAR(self), GET_MONTH(self), GET_DAY(self), 0,
                             0, 0, -1);
}
...
date_replace(PyDateTime_Date *self, PyObject *args, PyObject *kw)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kw, "|iii:replace", date_kws, &year,
                                     &month, &day))
        return NULL;
...
{
    if (self->hashcode == -1) {
        self->hashcode = generic_hash((unsigned char *)self->data,
                                      _PyDateTime_DATE_DATASIZE);
    }
...
date_toordinal(PyDateTime_Date *self)
{
    return PyLong_FromLong(
        ymd_to_ord(GET_YEAR(self), GET_MONTH(self), GET_DAY(self)));
}
...
static PyMethodDef date_methods[] = {
    ...
    {"fromtimestamp", (PyCFunction)date_fromtimestamp,
     METH_VARARGS | METH_CLASS,
     PyDoc_STR("timestamp -> local date from a POSIX timestamp (like "
               "time.time()).")},

    {"fromordinal", (PyCFunction)date_fromordinal, METH_VARARGS | METH_CLASS,
     PyDoc_STR("int -> date corresponding to a proleptic Gregorian "
               "ordinal.")},

    {"fromisoformat", (PyCFunction)date_fromisoformat, METH_O | METH_CLASS,
...
tzinfo_nogo(const char *methodname)
{
    PyErr_Format(PyExc_NotImplementedError,
                 "a tzinfo subclass must implement %s()", methodname);
    return NULL;
}
...
static PyObject *
timezone_richcompare(PyDateTime_TimeZone *self, PyDateTime_TimeZone *other,
                     int op)
{
...
timezone_str(PyDateTime_TimeZone *self)
{
    ...
    hours = divmod(minutes, 60, &minutes);
    if (microseconds != 0) {
        return PyUnicode_FromFormat("UTC%c%02d:%02d:%02d.%06d", sign, hours,
                                    minutes, seconds, microseconds);
    }
    if (seconds != 0) {
        return PyUnicode_FromFormat("UTC%c%02d:%02d:%02d", sign, hours,
                                    minutes, seconds);
    }
...
static PyGetSetDef time_getset[] = {{"hour", (getter)time_hour},
...
time_new(PyTypeObject *type, PyObject *args, PyObject *kw)
{
    ...
    if (PyArg_ParseTupleAndKeywords(args, kw, "|iiiiO$i", time_kws, &hour,
                                    &minute, &second, &usecond, &tzinfo,
                                    &fold)) {
...
time_repr(PyDateTime_Time *self)
{
    ...
    if (us)
        result =
            PyUnicode_FromFormat("%s(%d, %d, %d, %d)", type_name, h, m, s, us);
    else if (s)
...
time_isoformat(PyDateTime_Time *self, PyObject *args, PyObject *kw)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kw, "|s:isoformat", keywords,
                                     &timespec))
        return NULL;
    ...
    if (timespec == NULL || strcmp(timespec, "auto") == 0) {
        ...
        for (given_spec = 0; given_spec < Py_ARRAY_LENGTH(specs);
             given_spec++) {
    ...
    if (given_spec == Py_ARRAY_LENGTH(specs)) {
        ...
        result = PyUnicode_FromFormat(
            specs[given_spec][1], TIME_GET_HOUR(self), TIME_GET_MINUTE(self),
...
time_strftime(PyDateTime_Time *self, PyObject *args, PyObject *kw)
{
     ...
     */
    tuple = Py_BuildValue("iiiiiiiii", 1900, 1, 1, /* year, month, day */
                          TIME_GET_HOUR(self), TIME_GET_MINUTE(self),
                          TIME_GET_SECOND(self), 0, 1,
                          -1); /* weekday, daynum, dst */
    if (tuple == NULL)
        return NULL;
...
time_hash(PyDateTime_Time *self)
{
    if (self->hashcode == -1) {
        ...
        if (TIME_GET_FOLD(self)) {
            self0 = new_time_ex2(
                TIME_GET_HOUR(self), TIME_GET_MINUTE(self),
                TIME_GET_SECOND(self), TIME_GET_MICROSECOND(self),
                HASTZINFO(self) ? self->tzinfo : Py_None, 0, Py_TYPE(self));
            if (self0 == NULL)
                return -1;
        }
        else {
...
time_replace(PyDateTime_Time *self, PyObject *args, PyObject *kw)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kw, "|iiiiO$i:replace", time_kws,
...
time_fromisoformat(PyObject *cls, PyObject *tstr)
{
    ...
    if ((PyTypeObject *)cls == &PyDateTime_TimeType) {
        ...
        t = PyObject_CallFunction(cls, "iiiiO", hour, minute, second,
                                  microsecond, tzinfo);
    }
...
static PyMethodDef time_methods[] = {
    ...
    {"isoformat", (PyCFunction)time_isoformat, METH_VARARGS | METH_KEYWORDS,
     PyDoc_STR(
         "Return string in ISO 8601 format, [HH[:MM[:SS[.mmm[uuu]]]]]"
         "[+HH:MM].\n\n"
...
static char *datetime_kws[] = {"year",   "month",  "day",         "hour",
...
datetime_new(PyTypeObject *type, PyObject *args, PyObject *kw)
{
    ...
    if (PyArg_ParseTupleAndKeywords(args, kw, "iii|iiiiO$i", datetime_kws,
        ...
        self = new_datetime_ex2(year, month, day, hour, minute, second,
...
local(long long u)
{
    ...
    if (_PyTime_localtime(t, &local_time) != 0)
        return -1;
    return utc_to_seconds(local_time.tm_year + 1900, local_time.tm_mon + 1,
                           ...
                           PyObject *tzinfo)
{
    ...
    if (tzinfo == Py_None && f == _PyTime_localtime) {
        ...
        result_seconds =
            utc_to_seconds(year, month, day, hour, minute, second);
                        ...
                        PyObject *tzinfo)
{
    ...
    if (_PyTime_ObjectToTimeval(timestamp, &timet, &us,
                                _PyTime_ROUND_HALF_EVEN) == -1)
        return NULL;
...
/*[clinic end generated code: output=b3386e5345e2b47a input=80d09869c5267d00]*/
{
    ...
    self = datetime_best_possible(
        (PyObject *)type, tz == Py_None ? _PyTime_localtime : _PyTime_gmtime,
        tz);
...
datetime_fromtimestamp(PyObject *cls, PyObject *args, PyObject *kw)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kw, "O|O:fromtimestamp", keywords,
                                     &timestamp, &tzinfo))
        return NULL;
    if (check_tzinfo_subclass(tzinfo) < 0)
        return NULL;
    ...
    self = datetime_from_timestamp(
        cls, tzinfo == Py_None ? _PyTime_localtime : _PyTime_gmtime, timestamp,
        tzinfo);
...
datetime_utcfromtimestamp(PyObject *cls, PyObject *args)
{
    ...
    if (PyArg_ParseTuple(args, "O:utcfromtimestamp", &timestamp))
        result =
            datetime_from_timestamp(cls, _PyTime_gmtime, timestamp, Py_None);
    return result;
}
...
datetime_strptime(PyObject *cls, PyObject *args)
{
    ...
    return _PyObject_CallMethodIdObjArgs(module, &PyId__strptime_datetime, cls,
                                         string, format, NULL);
}
...
datetime_combine(PyObject *cls, PyObject *args, PyObject *kw)
{
    ...
    if (PyArg_ParseTupleAndKeywords(args, kw, "O!O!|O:combine", keywords,
        ...
        result = new_datetime_subclass_fold_ex(
            ...
            TIME_GET_MICROSECOND(time), tzinfo, TIME_GET_FOLD(time), cls);
    }
    return result;
}
                       ...
                       int factor)
{
    ...
    int microsecond =
        DATE_GET_MICROSECOND(date) + GET_TD_MICROSECONDS(delta) * factor;

    assert(factor == 1 || factor == -1);
    if (normalize_datetime(&year, &month, &day, &hour, &minute, &second,
                           &microsecond) < 0) {
        return NULL;
    }

    return new_datetime(year, month, day, hour, minute, second, microsecond,
...
datetime_subtract(PyObject *left, PyObject *right)
{
    ...
    if (PyDateTime_Check(left)) {
        ...
        if (PyDateTime_Check(right)) {
            ...
            delta_us =
                DATE_GET_MICROSECOND(left) - DATE_GET_MICROSECOND(right);
            ...
            /* datetime - delta */
            result = add_datetime_timedelta((PyDateTime_DateTime *)left,
                                            (PyDateTime_Delta *)right, -1);
        }
    }
...
datetime_repr(PyDateTime_DateTime *self)
{
    ...
    if (DATE_GET_MICROSECOND(self)) {
        baserepr = PyUnicode_FromFormat(
            "%s(%d, %d, %d, %d, %d, %d, %d)", type_name, GET_YEAR(self),
            GET_MONTH(self), GET_DAY(self), DATE_GET_HOUR(self),
            DATE_GET_MINUTE(self), DATE_GET_SECOND(self),
            DATE_GET_MICROSECOND(self));
    }
    else if (DATE_GET_SECOND(self)) {
        baserepr = PyUnicode_FromFormat(
            "%s(%d, %d, %d, %d, %d, %d)", type_name, GET_YEAR(self),
            GET_MONTH(self), GET_DAY(self), DATE_GET_HOUR(self),
            DATE_GET_MINUTE(self), DATE_GET_SECOND(self));
    }
    else {
        baserepr = PyUnicode_FromFormat("%s(%d, %d, %d, %d, %d)", type_name,
                                        GET_YEAR(self), GET_MONTH(self),
                                        GET_DAY(self), DATE_GET_HOUR(self),
...
datetime_isoformat(PyDateTime_DateTime *self, PyObject *args, PyObject *kw)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kw, "|Cs:isoformat", keywords, &sep,
                                     &timespec))
        return NULL;
    ...
    if (given_spec == Py_ARRAY_LENGTH(specs)) {
        ...
        result = PyUnicode_FromFormat(
            specs[given_spec][1], GET_YEAR(self), GET_MONTH(self),
            GET_DAY(self), (int)sep, DATE_GET_HOUR(self),
            DATE_GET_MINUTE(self), DATE_GET_SECOND(self), us);
    }
...
{
    return format_ctime((PyDateTime_Date *)self, DATE_GET_HOUR(self),
                        DATE_GET_MINUTE(self), DATE_GET_SECOND(self));
}
...
flip_fold(PyObject *dt)
{
    return new_datetime_ex2(
        GET_YEAR(dt), GET_MONTH(dt), GET_DAY(dt), DATE_GET_HOUR(dt),
        DATE_GET_MINUTE(dt), DATE_GET_SECOND(dt), DATE_GET_MICROSECOND(dt),
        HASTZINFO(dt) ? ((PyDateTime_DateTime *)dt)->tzinfo : Py_None,
        !DATE_GET_FOLD(dt), Py_TYPE(dt));
}
 ...
 */
static int
pep495_eq_exception(PyObject *self, PyObject *other, PyObject *offset_self,
                    PyObject *offset_other)
{
    int result = 0;
...
local_timezone_from_timestamp(time_t timestamp)
{
...
#else  /* HAVE_STRUCT_TM_TM_ZONE */
    {
        ...
        local_time = new_datetime(
            local_time_tm.tm_year + 1900, local_time_tm.tm_mon + 1,
            local_time_tm.tm_mday, local_time_tm.tm_hour, local_time_tm.tm_min,
        ...
        utc_time = new_datetime(utc_time_tm.tm_year + 1900,
                                utc_time_tm.tm_mon + 1, utc_time_tm.tm_mday,
                                utc_time_tm.tm_hour, utc_time_tm.tm_min,
...
static long long
local_to_seconds(int year, int month, int day, int hour, int minute,
                 int second, int fold);
...
local_timezone_from_local(PyDateTime_DateTime *local_dt)
{
    ...
    seconds = local_to_seconds(
        GET_YEAR(local_dt), GET_MONTH(local_dt), GET_DAY(local_dt),
        DATE_GET_HOUR(local_dt), DATE_GET_MINUTE(local_dt),
        DATE_GET_SECOND(local_dt), DATE_GET_FOLD(local_dt));
    if (seconds == -1)
        return NULL;
...
datetime_astimezone(PyDateTime_DateTime *self, PyObject *args, PyObject *kw)
{
    ...
    if (!HASTZINFO(result)) {
        ...
        result = (PyDateTime_DateTime *)new_datetime_ex2(
            GET_YEAR(result), GET_MONTH(result), GET_DAY(result),
            DATE_GET_HOUR(result), DATE_GET_MINUTE(result),
            DATE_GET_SECOND(result), DATE_GET_MICROSECOND(result),
            PyDateTime_TimeZone_UTC, DATE_GET_FOLD(result), Py_TYPE(result));
        Py_DECREF(temp);
        if (result == NULL)
            return NULL;
    }
    ...
    temp = (PyObject *)result;
    result = (PyDateTime_DateTime *)_PyObject_CallMethodIdObjArgs(
        tzinfo, &PyId_fromutc, temp, NULL);
    Py_DECREF(temp);
...
datetime_timetuple(PyDateTime_DateTime *self)
{
    ...
    return build_struct_time(GET_YEAR(self), GET_MONTH(self), GET_DAY(self),
                             DATE_GET_HOUR(self), DATE_GET_MINUTE(self),
                             DATE_GET_SECOND(self), dstflag);
}
...
datetime_timestamp(PyDateTime_DateTime *self)
{
    ...
    if (HASTZINFO(self) && self->tzinfo != Py_None) {
        ...
        long long seconds;
        seconds =
            local_to_seconds(GET_YEAR(self), GET_MONTH(self), GET_DAY(self),
                             DATE_GET_HOUR(self), DATE_GET_MINUTE(self),
                             DATE_GET_SECOND(self), DATE_GET_FOLD(self));
        if (seconds == -1)
            return NULL;
...
{
    return new_time(DATE_GET_HOUR(self), DATE_GET_MINUTE(self),
                    DATE_GET_SECOND(self), DATE_GET_MICROSECOND(self), Py_None,
                    DATE_GET_FOLD(self));
}
...
datetime_gettimetz(PyDateTime_DateTime *self)
{
    return new_time(DATE_GET_HOUR(self), DATE_GET_MINUTE(self),
                    DATE_GET_SECOND(self), DATE_GET_MICROSECOND(self),
                    GET_DT_TZINFO(self), DATE_GET_FOLD(self));
}
...
static PyMethodDef datetime_methods[] = {
    ...
    {"utcnow", (PyCFunction)datetime_utcnow, METH_NOARGS | METH_CLASS,
    ...
    {"strptime", (PyCFunction)datetime_strptime, METH_VARARGS | METH_CLASS,
    ...
    {"fromisoformat", (PyCFunction)datetime_fromisoformat, METH_O | METH_CLASS,
    ...
    {"isoformat", (PyCFunction)datetime_isoformat,
...
PyInit__datetime(void)
{
    ...
    /* Epoch */
    PyDateTime_Epoch =
        new_datetime(1970, 1, 1, 0, 0, 0, 0, PyDateTime_TimeZone_UTC, 0);
    ...
    if (us_per_ms == NULL || us_per_second == NULL || us_per_minute == NULL ||
        seconds_per_day == NULL)
        return NULL;
